...
### 2023-05-14
添加了simple band to band tunneling和trap assisted tunneling（表现为增强SRH）
趋势与实际数据接近，考虑可以进一步调参来和实际数据符合
### 2023-05-08
发现此前放在P5中的仿真结果，所使用的数学公式是错误的（原因是一篇综述把公式分子分母写反了）
...
---
### simple band to band tunneling
$R_{BTBT}=-B|F|^{2.5}exp(\frac{-F_0}{F})$
其中F为对应位置的电场强度，B和$F_0$为可调参数

---
仿真结果

![simple tunneling 230514.png](https://raser-1314796952.cos.ap-beijing.myqcloud.com/media/simple%20tunneling%20230514.png)

---
此前辐照后的IV结果[[2023-02-13-TRACK]]：400V之前是电流量程导致的吗？
如果数据没问题的话，I-V曲线形状和仿真结果非常相似
...
---
仿真结果：趋势上和测量结果接近，
所使用公式：$\Gamma=2\sqrt{3\pi}\frac{F}{3.9\times10^4}exp(\frac{F}{3.9\times10^4})^2$,
F是电场强度
![field enhanced srh 230515.png](https://raser-1314796952.cos.ap-beijing.myqcloud.com/media/field%20enhanced%20srh%20230515.png)
