...
---
一些想法
- 通过实验数据推测芯片工艺的一些固有性质（辐射半径，辐射长度）
- 通过仿真模拟，试着去看在这种性质下，最多能达到什么样的分辨率。
	- 比如25um制程可以实现5-7um分辨率，clustersize基本不超过4，所以还有不小提升空间
	- 如果提高到5um制程时，clustersize依旧主要在4左右，那就可以是一个有效的提升
	- 如果再提高制程，簇团尺寸远超于4，这时提高制程对于分辨率提升并没有太大的帮助
- 

---
- 已完成G4部分：
- geant4setting.json的格式大体上无需特别标注细节，但有些还需注意下：
	- 1.Layer的名字不要随便设定，括号内的Name和括号外的title要一致，因为在记录STEP的时候需要模块名称，我设定的是前缀Layer才被记录

- 已初步完成Telescope：
	- 主要功能包括：坐标转换，挑选簇团，重建径迹，统计残差分布计算分辨率
	- 还需改进的部分：
		- 1. 多参考已有的代码，主要是接口数据格式，簇团重建算法
		- 2. 目前使用了实验时的Telescope方案，并不是经常使用的默认的Telescope，需要多阅读文献和已有代码

- 参考EUTelescope和Allpix square
	- EUTelescope: clustering 部分除了算法外的簇团定义簇团中心位置计算是一致的
