        ...
        with open(laser_json) as f:
            ...
            for i in range(laser_dic['pos_points_num']+1):
                ...
                if (laser_dic["fx_rel"]>0.428) and  (laser_dic["fx_rel"]<0.5714):
                    ...
                    path = output(__file__, my_d.det_name, my_l.model+'position')
...
def main(kwargs):
    ...
    for i in range(scan_number):
        command = ' '.join(['python3', 'raser', '-b', 'tct position_signal',sys.argv[3],sys.argv[4], '--job', str(i)] + sys.argv[5:]) # 'raser', '-sh', 'gen_signal'
